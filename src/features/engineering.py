...
def generate_features(bars: list[MarketBar]) -> list[dict[str, object]]:
    ...
    for symbol, series in by_symbol.items():
        ...
        for bar in series:
            ...
            ma_5 = sum(list(closes)[-5:]) / min(len(closes), 5)
            vol_20 = (max(closes) - min(closes)) / bar.close if len(closes) > 1 else 0.0
            ...
            feature_rows.append(
                {
                    ...
                    "range_20": round(vol_20, 8),
                    "volume_ratio": round((bar.volume / avg_vol_20) if avg_vol_20 else 0.0, 8),
                }
            )
            prev_close = bar.close
    return sorted(feature_rows, key=lambda x: (x["symbol"], x["date"]))
