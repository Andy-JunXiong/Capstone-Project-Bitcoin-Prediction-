...
import { forenameColor, getDateString } from './utils'
...
export default {
  ...
  methods: {
    ...
    countForForename (forenameData) {
      const d = _.find(forenameData.prices, d => d.year === this.year)
      if (d) {
        return forenameData.name + ', price: ' + d.prices + ', at day: ' + getDateString(d.year)
      } else {
        return ''
      }
    },
