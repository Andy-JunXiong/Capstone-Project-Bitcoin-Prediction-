...
.svg-container
  .graph
    ...
    .year-zoomers
      a(
        href="#"
        v-for="r,i in yearRanges"
        @click.prevent="yearClick(r,i)",
        :class="{selected: zoomed && range.from === r.from}")
        span.year  {{r.from}} → {{r.to}}
  .labels(ref="labels")
  popup(:forenames="forenames", :position="popup.position", :show="popup.show", :year="currentYear")
</template>

<script>
import * as d3 from 'd3'
import _ from 'lodash'
import { defaultDuration, forenameColor, years, maxPricesCount, initialRange, timestart, getDateString } from './utils'
...
export default {
  ...
  mounted () {
    ...
    this.$el.addEventListener('mouseenter', (evt) => {
      this.popup.show = this.forenames.length > 0
    ...
    this.$el.addEventListener('mousemove', (evt) => {
      ...
      this.svg
        .selectAll('.circles > g')
        .attr('visibility', d => d.year === this.currentYear ? '' : 'hidden')
    }, true)
  ...
  methods: {
    yearClick (clickedRange, index) {
      let from = years[index * 10]
      let to = years[index * 10 + 10]
      if (this.zoomed && this.range.from === from) {
        this.range = initialRange
        this.zoomed = false
      } else {
        this.range = { from: from, to: to + 1 }
        this.zoomed = true
      }
    ...
    redrawExistingLines (lines) {
      lines
        ...
        .select('g path')
        .attr('d', (prenomData) => this.lineGenerator(prenomData.prices))
    },
    ...
    placeLabel (labels) {
      ...
      const css = []
      labels
        .each(function (forenameData, i) {
          const { year, prices } =
            _(forenameData.prices)
              .filter(({ year }) => year >= self.range.from && year <= self.range.to - 1)
              .maxBy('prices')

          // console.log(this.getBoundingClientRect())
          const top = self.yScale(prices)
      ...
      labels
        ...
        .style('background-color', forenameColor)
    },
    ...
    transformForCircle (d) {
      ...
      const y = this.yScale(d.prices)
      return `translate(${x} ${y})`
    },

    updateAxis () {
      let ranges = _.range(this.range.from, this.range.to, 5)
      this.xScale.domain([this.range.from, this.range.to - 1])
      this.xAxis.tickValues(ranges)
      this.xAxis.tickFormat(day => getDateString(day))
      // this.xAxis.tickFormat(d3.format(".0%"))
      const maxPrices = Math.max(1000, maxPricesCount(this.forenames, this.range) * 1.1)
      console.log(maxPrices)
      if (maxPrices > 5000) {
        this.yScale.domain([5000, maxPrices])
      } else {
        this.yScale.domain([maxPrices, 5000])
      }
    },
    ...
    updateLines () {
      ...
      const initialLine = this.lineGenerator(years.map(year => ({ year, prices: 0 })))

      const lines =
        this.svg
          .select('.prenoms .lines')
          .selectAll('g')
          .data(this.forenames, d => d.id)
      ...
      newLines
        .append('path')
        .attr('stroke', forenameColor)
        .style('clip-path', (prenomData, i) => `url(#${clipPathId(prenomData, i)})`)
        // .attr('d', initial_line)
        // .transition()
        // .duration(defaultDuration)
        .attr('d', (d) => {
          return this.lineGenerator(d.prices)
        })
      ...
      const circleData = _.flatMap(this.forenames, (forenameData) => {
        const color = forenameColor(forenameData)
        return _.map(forenameData.prices, ({ prices, year }) => {
          return { color, year, prices, id: forenameData.id + '-' + year }
        })
      })
      ...
      disappear(labels)
        .attr('transform', prenomData => {
          ...
          prenomData.prices.forEach(({ price, year }) => {
  ...
  computed: {
    yearRanges () {
      return _(years[0])
        ...
        .map(from => ({ from: getDateString(from), to: getDateString(from + 10) }))
        .value()
    }
  }
